...
String BOTtoken = "6022091643:AAGcSIDYTo8evpajaxxHi7rfrH9tqHU8NZI";
//your Bot Token(Get from Botfather)
// Use @myidbot to find out the chat ID of an individual or a group
// Also note that you need to click "start" on a bot before it can
// message you
String CHAT_ID = "609643010";
...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (text == "/motion_on") {
      motionEnabled = true;
      bot.sendMessage(CHAT_ID, "Motion notifications ON.", "");
    }
    if (text == "/motion_off") {
      motionEnabled = false;
      bot.sendMessage(CHAT_ID, "Motion notifications OFF.", "");
    }
    if (text == "/motion_state") {
      if (motionEnabled == false) {
        bot.sendMessage(CHAT_ID, "Motion notifications are OFF.",
                        "");
      } else {
        bot.sendMessage(CHAT_ID, "Motion notifications are ON.",
                        "");
      }
    }
    if (text == "/start") {
